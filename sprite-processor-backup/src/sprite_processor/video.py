...
try:
    # Correct import - VideoFileClip is directly from moviepy in newer versions
...
def extract_gif_frames_to_dir(
    gif_path: Path, output_dir: Path, max_frames: int | None = None
) -> list[Path]:
    """
...
def video_to_spritesheet(
    ...
    *,
    ...
    try:
        ...
        from .cli import (
            _create_spritesheet,  # expects (images, cols, rows, output_path) -> Path
        )
