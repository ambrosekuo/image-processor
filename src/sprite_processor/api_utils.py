...
async def save_uploaded_file(file: UploadFile, suffix: str | None = None) -> Path:
    """
...
def parse_grid(grid: str) -> tuple[int, int]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Grid numbers must be positive integers."
        ) from None
...
def extract_gif_frames(gif_path: Path, max_frames: int | None = None) -> list[Image.Image]:
    """
    ...
    """
    from .video import extract_gif_frames as extract_gif_frames_composited

    frames = extract_gif_frames_composited(gif_path, max_frames=max_frames, sample_evenly=False)
    if not frames:
...
def create_spritesheet(
    ...
    for i, frame in enumerate(frames):
        ...
        if frame.mode == "RGBA":
            ...
            combined.paste(frame.convert("RGBA"), (c * fw, r * fh))
    return combined
...
def create_success_response(
    data: dict[str, Any], filename: str | None = None, **kwargs
) -> dict[str, Any]:
    """
...
def create_error_response(error: str, status_code: int = 500, **kwargs) -> HTTPException:
    """
