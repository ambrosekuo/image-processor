...
def one(input: Path, output: Path | None, overwrite: bool) -> None:
...
def _create_spritesheet(frames: list[Image.Image], cols: int, rows: int, output_path: Path) -> Path:
    """
    Create a spritesheet from a list of PIL Images.

    Args:
        frames: List of PIL Images to arrange in the spritesheet
        cols: Number of columns in the grid
        rows: Number of rows in the grid
        output_path: Path where the spritesheet will be saved

    Returns:
        Path to the created spritesheet
    """
    if not frames:
...
@click.argument("output_dir", type=click.Path(file_okay=False, path_type=Path))
@click.option(
    "--frame-width", type=int, help="Width of each frame in pixels (alternative to --grid)"
)
@click.option(
    "--frame-height", type=int, help="Height of each frame in pixels (alternative to --grid)"
)
@click.option(
    "--grid",
    type=str,
    help="Grid layout as 'WIDTHxHEIGHT' (e.g., '5x2' for 5 frames per row, 2 rows)",
)
@click.option(
    "--frames", type=int, help="Total number of frames to process (for irregular layouts)"
)
@click.option(
    "--frames-per-row", type=int, help="Number of frames per row (auto-detect if not specified)"
)
@click.option(
    "--output-spritesheet",
    type=click.Path(dir_okay=False, path_type=Path),
    help="Output as a single spritesheet file instead of individual frames",
)
...
def video(
    ...
    try:
        ...
        result_path = video_to_gif(
            input, output, fps=fps, duration=duration, max_width=max_width, max_height=max_height
        )
...
@click.argument("input", type=click.Path(exists=True, path_type=Path))
@click.option(
    "-o", "--output", type=click.Path(path_type=Path), help="Output spritesheet file path"
)
...
def video_spritesheet(
    ...
    try:
        ...
        result_path = video_to_spritesheet(
            input, output, grid=grid, frames=frames, fps=fps, duration=duration
        )
...
@click.option(
    "--keep-intermediates", is_flag=True, help="Keep intermediate files (GIF, spritesheet)"
)
@click.option("--overwrite", is_flag=True, help="Overwrite existing output files")
def pipeline(
    input: Path,
