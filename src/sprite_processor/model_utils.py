...
async def process_single_model(image_data: bytes, model_name: str) -> dict[str, any]:
    """
...
async def process_all_models(
    image_data: bytes, models: list[str] | None = None
...
async def process_frame_with_model(
    frame_path: Path, output_path: Path, model_name: str = "isnet-general-use"
) -> Image.Image:
    """
    ...
    for i, frame in enumerate(frames):
        try:
            ...
            processed_frame = process_frame_with_model(
                temp_frame_path, Path(f"temp_processed_{i}.png"), model_name
            )
            ...
            for i, frame in enumerate(frames):
                try:
                    ...
                    processed_frame = await process_frame_with_model(
                        temp_frame_path, Path(f"temp_processed_{model}_{i}.png"), model
                    )
                    processed_frames.append(processed_frame)
                    temp_frame_path.unlink(missing_ok=True)
