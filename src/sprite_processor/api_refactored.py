...
from .model_utils import process_all_models
...
async def analyze_spritesheet_endpoint(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise create_error_response(str(e)) from None
    finally:
...
async def remove_all_models_endpoint(file: UploadFile = File(...)):
    ...
    try:
        ...
        return create_success_response(
            {"original_filename": file.filename, "original_size": len(data), "models": results}
        )
    except Exception as e:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if input_is_gif:
                ...
                for i, frame in enumerate(extracted_frames):
                    ...
                    processed = await process_frame_with_model(in_frame_path, out_frame_path, model)
...
async def process_video_to_gif_endpoint(
    ...
    try:
        ...
        raise create_error_response(f"Video processing failed: {str(e)}") from None
...
async def analyze_video_endpoint(file: UploadFile = File(...)):
    ...
    try:
        ...
        return create_success_response({"filename": file.filename, "analysis": analysis})
    except Exception as e:
        logger.error(f"Video analysis failed: {e}")
        raise create_error_response(f"Video analysis failed: {str(e)}") from None
    finally:
        cleanup_temp_files(temp_file_path)
...
async def process_video_pipeline_endpoint(
    ...
    try:
        ...
        raise create_error_response(f"Video pipeline failed: {str(e)}") from None
    finally:
        cleanup_temp_files(temp_file_path)
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            frame_paths = extract_gif_frames_to_dir(
                gif_path=temp_file_path, output_dir=output_dir, max_frames=frames
            )
            if not frame_paths:
                raise HTTPException(
            ...
            for i, frame_path in enumerate(frame_paths):
                try:
                    ...
                    processed_frame = await process_frame_with_model(frame_path, processed_path)
        ...
        raise create_error_response(f"GIF to spritesheet conversion failed: {str(e)}") from None
    finally:
        cleanup_temp_files(temp_file_path)
